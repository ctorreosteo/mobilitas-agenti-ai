...
├── scripts/sync-repos.sh     # clone/fetch dei repo nel workspace
...
└── .claude/
    ...
    ├── hooks/blocca-modifiche-codice.py
...
git -C /Users/carlitos/agente-gdpr/workspace/mobilitas-backend \
    diff --stat main...gdpr/aggiornamento-docs
...
1. **Hook `PreToolUse`** su `Write|Edit|MultiEdit|NotebookEdit`: blocca ogni scrittura che non
   sia un `.md` dentro `workspace/`.
2. **Hook `PreToolUse` su Bash**: blocca `git push`, riscritture in-place (`sed -i`,
   `tee`, redirezioni) verso file di codice del workspace.
3. **Deny list** in `.claude/settings.json` sulle estensioni di codice.
4. **Regola prima in `CLAUDE.md`**: le criticità del codice si *segnalano*, non si correggono.
